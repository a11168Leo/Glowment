...
- `migrations/` — a estrutura da base de dados, um ficheiro por tema, **por ordem**:
  ...
  | 10 | `10_pesquisa.sql` | Pesquisa (sem acentos, tolera erros) e categorias de serviço |

- `testes/testes_base_de_dados.sql` — 43 testes automáticos (segurança e agenda).
...
3. Repete para os ficheiros 2 a 10, **por esta ordem**.
4. Corre `testes/testes_base_de_dados.sql`. Deve aparecer
   **"✅ TODOS OS TESTES PASSARAM (43/43)"** (aparece como erro de propósito: é assim que o teste se desfaz).
...
A função `pesquisar_saloes` procura pelo nome do salão, cidade, nome ou categoria de serviço,
ignorando acentos e maiúsculas e tolerando erros ("barbaria" encontra "Barbearia").
No site: `pesquisarSaloes({ texto, local, categoria, categoriaServico })` em `frontend/src/servicos/saloes.js`.

## Mudanças futuras

Nunca alterar um ficheiro que já foi aplicado. Cria-se um novo em `migrations/`
com o número seguinte (ex.: `11_lista_de_espera.sql`), acrescenta-se
